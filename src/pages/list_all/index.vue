...
export default {
  ...
  setup(){
    ...
    let paginationData = inject('paginationDataListAll');
    let sortData       = inject('sortDataListAll');
    let filterData     = inject('filterDataListAll');
    let showValueFilter= inject('showValueListAllFilter');
    let activeProfile  = inject('activeProfileListAll');

    const router       = useRouter();

    const loading      = computed(() => { return dataGrid.loading });

    const name         = ref('smartCardListAll');
    ...
    const content      = reactive({
      ...
      context         : [
        ...
        // {
        //   text: 'Редактировать SMART-карту',
        //   function: (e, row) => {
        //
        //     if (row.row.status_eng[0].value != 'created') return notify('Редактирование SMART-карты', `Редактирование допускается только на этапе подготовки SMART-карты! Текущий статус SMART-карты - ${row.row.status[0].value}.`, 'error');
        //
        //     ElMessageBox.confirm(`Вы уверены, что хотите редактировать SMART-карту с ID - ${row.row.id[0].value}?`)
        //         .then(() => {
        //           smartCardId.value = row.row.id[0].value;
        //           router.push({name:'smartCardEdit', params: { id: row.row.id[0].value }});
        //         })
        //         .catch(() => {})
        //   }
        // },
        // {
        //   text: 'Удалить SMART-карту',
        //   function: (e, row) => {
        //     if (row.row.status_eng[0].value != 'created') return notify('Удаление SMART-карты', `Удаление SMART-карты допускается только на этапе подготовки! Текущий статус SMART-карты - ${row.row.status[0].value}.`, 'error');
        //
        //     ElMessageBox.confirm(`Вы уверены, что хотите удалить SMART-карту с ID - ${row.row.id[0].value}?`)
        //         .then(() => {
        //           smartCardId.value = row.row.id[0].value;
        //           deleteSmartCard();
        //         })
    ...
    async function getData(filter = filterData, sort = sortData) {
      ...
      let gridElements = [];
      let result = await loadJson('/smart-card/list', {
    ...
    async function deleteSmartCard(){
      dataGrid.loading = true;
      let result = await loadJson('/smart-card/delete', {
        user_id      : userId.value,
    ...
    return{
      svg, loading, name,
      getData, sendDefaultFilterData, deleteSmartCard
    }
  }
}
</script>
